...
function getAgeProfile(age: number): AgeProfile {
  if (age <= 4) {
    return {
      wordRange: '180–230 слов',
      complexity: `Предложения короткие, НО с ритмом и чередованием: 2–3 коротких → одно чуть длиннее → снова короткие. ЗАПРЕЩЕНО писать подряд более трёх предложений одинаковой длины.

ОБЯЗАТЕЛЬНО для этого возраста:
— Минимум 3 сенсорных детали (мягкий, тёплый, щекотно, пахнет, блестит, шуршит) — в разных частях истории
— Минимум 2 восклицания (от ребёнка, от героя или от обоих)
— Объяснение через ДЕЙСТВИЕ — герой что-то делает, показывает, предлагает попробовать
— Диалог объяснения — МИНИМУМ 3 обмена репликами (вопрос → ответ → уточнение → ответ → реакция), не один
— Ребёнок должен задать хотя бы ОДИН уточняющий вопрос после первого ответа героя

ЗАПРЕЩЕНО:
— Объяснение в одну реплику: "Вот зачем! — Правда? — Да!" — это не диалог, это обрыв
— Писать подряд: "X сделал. Y сказал. Z улыбнулась." — это протокол, не сказка
— Использовать одно и то же описательное выражение дважды в одной истории
— Объяснять факт напрямую — только через образ, показ или маленькую историю`,
      style: 'как любимая книжка перед сном — с ритмом, тёплыми образами и маленьким чудом в каждом абзаце',
    };
  } else if (age <= 6) {
    return {
...
- Между предложениями — плавные переходы. Даже у малышей текст должен течь как ручеёк, а не прыгать
...
- Описания богатые: не "слушала", а "слушала, затаив дыхание" / "слушала, прижав ушки"; не "смотрела", а "смотрела широко раскрытыми глазами"
...
- Появление УКАЗАННОГО героя-помощника — он появляется ПОДГОТОВЛЕННО: сначала намёк (звук, запах, тень, голос), потом сам герой. ЗАПРЕЩЕНО: "[Имя] пришёл. Он сказал:" — это обрывает магию.
ТЕХНИКА появления (придумай свой вариант, не копируй структуру): сначала — какой-то знак присутствия героя (звук / тень / блеск / запах), потом — деталь внешности, потом — герой целиком с одной живой характеристикой.
- ГЛАВНАЯ ЧАСТЬ (самая длинная!) — УКАЗАННЫЙ герой ОТВЕЧАЕТ на вопрос через волшебную метафору или мини-историю внутри истории. Ребёнок задаёт уточняющие вопросы, герой отвечает. Объяснение должно быть научно верным, но подано через образ или ПОКАЗ — никогда через сухой факт.
ТЕХНИКА объяснения: герой что-то ДЕЛАЕТ (демонстрирует, рассказывает историю, задаёт вопрос ребёнку в ответ, показывает пальцем или рисует в воздухе) — ответ приходит через переживание, а не через слова-факты. Ребёнок должен сам «дойти» до ответа вместе с героем через 2–3 шага диалога.
- Тёплый финал — ребёнок понял ответ, улыбается, чувствует себя умнее и счастливее. Финал НЕ заканчивается фактом или выводом. Он заканчивается ощущением: теплом, уютом, маленьким чудом. Последнее предложение — образное, не информативное.

ЛОГИКА И НЕПРЕРЫВНОСТЬ — КРИТИЧЕСКИ ВАЖНО:
- Все персонажи подчиняются физическому миру: если игрушка лежит рядом — она там и остаётся. Нельзя написать "игрушка ждала дома", если её не уносили домой — это разрушает логику истории
- Каждый переход между сценами — плавный. Если место меняется — покажи как: "они пошли домой", "солнце уже заходило, и пора было возвращаться"
- Если игрушка участвует в сцене — пусть остаётся в кадре до конца или её уход объяснён
- Никаких телепортаций: герой не может "вдруг оказаться" в новом месте без перехода
...
ЗАПРЕТ НА ПОВТОРЫ — СТРОГО:
- Нельзя использовать одно и то же описательное выражение дважды в одной истории ("широко раскрытыми глазами" — только один раз, "улыбнулась" — меняй на синонимы: засмеялась / просияла / обрадовалась)
- Нельзя использовать одну и ту же реакцию для двух разных персонажей в одной сцене
...
ОБЯЗАТЕЛЬНЫЙ герой сказки (используй ТОЧНО это имя): ${hero.name}${shouldUseToys && child.toys?.length > 0 ? `\nОБЯЗАТЕЛЬНО включи в сказку любимую игрушку ребёнка (выбери одну): ${child.toys.map(t => `${t.nickname} — ${t.type}${t.description ? ', ' + t.description : ''}`).join('; ')}. Игрушка должна быть живым участником сцены — рядом лежит, смотрит, слушает. Она остаётся там, где была, до конца истории или её уход объяснён. ЗАПРЕЩЕНО: говорить "ждала дома" если игрушку домой не уносили. ЗАПРЕЩЕНО придумывать другие игрушки.` : ''}
